...
const ForecastCard = ({ data, isFirst }) => {
  const boxBg = useColorModeValue(
    isFirst ? "teal.100" : "gray.100",
    isFirst ? "teal.600" : "gray.700"
  );
  ...
  return (
    ...
    >
      <Heading
        as={isFirst ? "h2" : "h3"}
        size={isFirst ? "lg" : "md"}
        color={isFirst ? textColorFirst : textColor}
        marginBottom={4}
      >
        {isFirst
          ? `Today: ${data.location.location_name}`
          : data.location.location_name}
      </Heading>
      {renderForecastItem(<FaCalendarAlt />, "Date", data.date)}
      {renderForecastItem(
        getWeatherIcon(data.morning_forecast),
        "Morning Forecast",
        data.morning_forecast
      )}
      {renderForecastItem(
        getWeatherIcon(data.afternoon_forecast),
        "Afternoon Forecast",
        data.afternoon_forecast
      )}
      {renderForecastItem(
        getWeatherIcon(data.night_forecast),
        "Night Forecast",
        data.night_forecast
      )}
